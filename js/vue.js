...
$(window).on('load', function () {
  ...
  } else { console.log("WEBP Images Supported"); }
...
Vue.component('Thought', {
  props: ['item'],
  methods: { getClass: function (size) { return "card shadow col-md-" + size + " mt-5" } },
  template: `  
  ...
  methods: {
    GetImage(img) {
      ...
      switch (this.WebPSupport) {
        case 1: return img;
        case 0: return img + ".png";
        default:
          let elem = document.createElement('canvas');
          if (!!(elem.getContext && elem.getContext('2d')) && elem.toDataURL('image/webp').indexOf('data:image/webp') == 0) {
            SupportsWebp = 1; return img;
          } else { SupportsWebp = 0; return img + "png"; }
      }
    },
...
/*Vue Items */
backend = new Vue({ el: "#vbackend", data: { Ex: pageObject.Experence, WepPSupport: SupportsWebp } });
...
blogs = new Vue({
  ...
  created() {
    fetch("https://dev.to/feed/liukonen")
      ...
      .then(data => { this.BlogItems = data.querySelectorAll("item"); });
  }
